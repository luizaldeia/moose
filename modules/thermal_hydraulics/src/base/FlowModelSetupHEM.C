...
#include "FlowModelSetupHEM.h"
#include "MooseObjectAction.h"

InputParameters
FlowModelSetupHEM::validParams()
{
  ...
  params.addRequiredParam<UserObjectName>("fp_hem", "HEM fluid properties object name");

  return params;
}

FlowModelSetupHEM::FlowModelSetupHEM(const InputParameters & params)
  : FlowModelSetup(params),
    ...
    _fp_hem_name(getParam<UserObjectName>("fp_hem")),
...
void
FlowModelSetupHEM::addInitialConditions()
{
  ...
  // rho
  {
    ...
    action->getObjectParams().set<UserObjectName>("fp") = _fp_hem_name;
...
void
FlowModelSetupHEM::addSolutionVariables()
{
...
void
FlowModelSetupHEM::addNonConstantAuxVariables()
{
  // area
...
void
FlowModelSetupHEM::addMaterials()
{
  ...
  // fluid properties
  {
    InputParameters params = _this_action_factory.getValidParams(class_name);
    if (_ad)
      params.set<std::string>("type") = "ADFluidPropertiesHEM3EqnMaterial";
    else
  ...
  // dynamic viscosity
  {
    ...
    InputParameters params = _this_action_factory.getValidParams(class_name);
    if (_ad)
      params.set<std::string>("type") = "ADDynamicViscosityHEMMaterial";
    else
    ...
    action->getObjectParams().set<UserObjectName>("fp_hem") = _fp_hem_name;
...
void
FlowModelSetupHEM::addUserObjects()
{
}
