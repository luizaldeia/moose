...
registerMooseObject("ThermalHydraulicsApp", FlowChannelHEM);
...
FlowChannelHEM::FlowChannelHEM(const InputParameters & params)
...
FlowChannelHEM::init()
{
  ...
  if (dynamic_cast<const HEM *>(&fp) == nullptr)
...
FlowChannelHEM::addHydraulicDiameterMaterial()
{
  ...
  if (isParamValid("D_h"))
  {
    ...
    params.set<std::vector<std::string>>("prop_names") = {FlowModelHEM::HYDRAULIC_DIAMETER};
  ...
  else
  {
    ...
    InputParameters params = _factory.getValidParams(class_name);
    params.set<std::vector<SubdomainName>>("block") = getSubdomainNames();
    params.set<MaterialPropertyName>("D_h_name") = FlowModelHEM::HYDRAULIC_DIAMETER;
