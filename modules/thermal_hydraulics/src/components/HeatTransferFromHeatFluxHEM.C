...
#include "HeatTransferFromHeatFluxHEM.h"
#include "FlowModelHEM.h"

registerMooseObject("ThermalHydraulicsApp", HeatTransferFromHeatFluxHEM);

InputParameters
HeatTransferFromHeatFluxHEM::validParams()
{
...
HeatTransferFromHeatFluxHEM::HeatTransferFromHeatFluxHEM(const InputParameters & parameters)
  : HeatTransferHEMBase(parameters), _q_wall_fn_name(getParam<FunctionName>("q_wall"))
{
  std::cout << "teste 2\n";
}

void
HeatTransferFromHeatFluxHEM::addMooseObjects()
{
...
bool
HeatTransferFromHeatFluxHEM::isTemperatureType() const
{
  std::cout << "teste 6\n";
  return false;
}
