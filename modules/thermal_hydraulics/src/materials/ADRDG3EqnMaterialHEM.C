...
#include "ADRDG3EqnMaterialHEM.h"
#include "HEM.h"
#include "THMIndices3EqnHEM.h"
#include "FlowModelHEMUtils.h"

registerMooseObject("ThermalHydraulicsApp", ADRDG3EqnMaterialHEM);

InputParameters
ADRDG3EqnMaterialHEM::validParams()
{
...
ADRDG3EqnMaterialHEM::ADRDG3EqnMaterialHEM(const InputParameters & parameters)
  : Material(parameters),
...
{
  _U_vars.resize(THM3EqnHEM::N_CONS_VAR - 1);
  _U_vars[THM3EqnHEM::CONS_VAR_RHOA] = _rhoA_var;
  _U_vars[THM3EqnHEM::CONS_VAR_RHOUA] = _rhouA_var;
  _U_vars[THM3EqnHEM::CONS_VAR_RHOEA] = _rhoEA_var;
  _U_vars[THM3EqnHEM::CONS_VAR_AREA] = _A_var;
}

void
ADRDG3EqnMaterialHEM::computeQpProperties()
{
  if (_scheme == None)
  {
  ...
  else
  {
    ...
    std::vector<ADReal> U_avg(THM3EqnHEM::N_CONS_VAR - 1, 0.0);
    U_avg[THM3EqnHEM::CONS_VAR_RHOA] = _rhoA_avg[_qp];
    U_avg[THM3EqnHEM::CONS_VAR_RHOUA] = _rhouA_avg[_qp];
    U_avg[THM3EqnHEM::CONS_VAR_RHOEA] = _rhoEA_avg[_qp];
    U_avg[THM3EqnHEM::CONS_VAR_AREA] = _A_avg[_qp];
    auto W = FlowModelHEMUtils::computePrimitiveSolutionVector<true>(U_avg, _fp);
    ...
    for (unsigned int m = 0; m < THM3EqnHEM::N_PRIM_VAR; m++)
      W[m] = W[m] + slopes[m] * delta_x;

    // compute reconstructed conservative variables
    const auto U =
        FlowModelHEMUtils::computeConservativeSolutionVector<true>(W, _A_linear[_qp], _fp);
    _rhoA[_qp] = U[THM3EqnHEM::CONS_VAR_RHOA];
    _rhouA[_qp] = U[THM3EqnHEM::CONS_VAR_RHOUA];
    _rhoEA[_qp] = U[THM3EqnHEM::CONS_VAR_RHOEA];

    std::cout << "\n";
    std::cout << "========================================\n";
    std::cout << "ADRDG3EqnMaterialHEM \n";
...
std::vector<ADReal>
ADRDG3EqnMaterialHEM::computeElementPrimitiveVariables(const Elem * elem) const
{
  const auto U = FlowModelHEMUtils::getElementalSolutionVector<true>(elem, _U_vars, _is_implicit);
  return FlowModelHEMUtils::computePrimitiveSolutionVector<true>(U, _fp);
}
